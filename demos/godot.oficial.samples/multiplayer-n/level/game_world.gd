...
remote func spawn_players(pinfo, spawn_index):
	...
	var pclass = load(pinfo.actor_path) # Load the scene and create an instance
	var nactor = pclass.instance()
	nactor.set_dominant_color(pinfo.char_color) # Setup player customization (well, the color)
	nactor.position = $SpawnPoints.get_node(str(spawn_index)).position # And the actor position
